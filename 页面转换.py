...
if __name__ == "__main__":
    ...
    def printCalender(self,year,month):
        _translate = QtCore.QCoreApplication.translate
        Calender = self.getDate(year,month)
        self.date = []
        d = 0
        for i in range(len(Calender)):
            for j in range(len(Calender[i])):
                if Calender[i][j] == 0:
                    ...
                    d += 1
                tmp = QtWidgets.QPushButton(self.widget)
                tmp.setGeometry(QtCore.QRect(50+j*60, 70+i*40, 21, 16))
                tmp.setObjectName("pushButton" + str(d))
                tmp.setText(_translate("MainWindow", str(d)))
